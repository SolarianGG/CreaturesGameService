...
- Document-level parts: `info` "GameService API" / `v1` (D-176), `servers: [{url: "/"}]` (D-180), no root-level `security` — each operation declares its own (D-183); Actuator and `/error` are not in the document (D-184, D-187).
